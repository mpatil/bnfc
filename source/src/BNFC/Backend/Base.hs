...
data GeneratedFile = GeneratedFile
  ...
  , writeOnce   :: Bool
      -- ^ If true, do not overwrite an existing file.
  , fileContent :: String
...
instance Show GeneratedFile where
  show (GeneratedFile x _ _ y) = unwords [ "GeneratedFile", show x, "_", show y ]
...
mkfile path f content = tell [GeneratedFile path f False (fileContentToString content)]

-- | Like 'mkfile', but existing files are preserved.
mkfileOnce :: FileContent c => FilePath -> MakeComment -> c -> MkFiles ()
mkfileOnce path f content = tell [GeneratedFile path f True (fileContentToString content)]
...
writeFiles root fw = do
  ...
  forM_ fb $ \ (GeneratedFile path mkComment once content) -> do
    ...
    if once
      then do
        ...
        if exists
          then putStrLn $ "preserving existing file " ++ (root </> path)
          else writeFileRep (root </> path) stamped
      else
        writeFileRep (root </> path) stamped
